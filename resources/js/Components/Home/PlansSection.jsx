...
const PlanCard = ({ plan }) => {
  ...
  return (
    <div className="flex flex-col h-full">
      <div 
        className="group relative rounded-[2.5rem] p-8 md:p-10 bg-gradient-to-b from-velonet-purple-light/50 to-velonet-purple border border-white/10 transition-all duration-700 hover:-translate-y-2 text-center overflow-hidden flex-1 flex flex-col"
        style={{ boxShadow: '2px 2px 20px 10px #b34ad45e inset' }}
      >
        {/* Top Badge */}
        {plan.badge && (
          <div className="absolute top-8 left-1/2 -translate-x-1/2 w-full px-10">
            <div className="bg-velonet-orange rounded-full py-2 px-4 shadow-lg">
              <span className="text-velonet-purple font-black uppercase text-[10px] tracking-tight block leading-tight">
                {plan.badge}
              </span>
            </div>
          </div>
        ...
        <div className="mt-14 mb-8 flex items-center justify-center gap-4">
          ...
          <div className="text-left">
            ...
            <p className="text-velonet-orange font-black text-[10px] uppercase tracking-widest">Para tu hogar</p>
          </div>
        </div>
        ...
        <div className="mb-10 flex items-center justify-center gap-3">
          ...
          {plan.promo ? (
            <div className="bg-velonet-orange rounded-xl py-3 px-6 flex items-center gap-3 shadow-lg transform group-hover:scale-110 transition-transform duration-500">
              ...
              <span className="text-velonet-purple font-black uppercase text-[12px] tracking-tighter leading-tight text-left">
                {plan.promo.split(' ')[0]}<br />{plan.promo.split(' ').slice(1).join(' ')}
              </span>
            </div>
          ) : (
            <div className="bg-[#4B2C6F] border border-white/10 rounded-xl py-3 px-6 flex items-center gap-1">
               <span className="text-white font-black text-xl tracking-tighter italic">DGO</span>
               <span className="text-velonet-orange font-black text-xl tracking-tighter italic">FLEX</span>
            </div>
          )}
        </div>
        ...
        <div className="flex items-start justify-center mb-10 text-white">
          <span className="text-lg font-black mt-2 mr-1 text-velonet-orange">$</span>
        ...
        <div className="mt-auto">
          <button className="w-full bg-velonet-orange text-velonet-purple py-4 rounded-[2rem] font-black uppercase tracking-[0.1em] text-[12px] flex items-center justify-center gap-3 hover:bg-white transition-all duration-300 shadow-xl shadow-velonet-orange/20 mb-8 font-outfit">
            Compra ahora <MessageCircle size={20} />
          </button>

          <button 
            onClick={() => setShowBenefits(!showBenefits)}
            className="text-white/40 hover:text-white font-black uppercase text-[10px] tracking-[0.2em] underline decoration-velonet-orange/30 underline-offset-8 transition-all flex items-center justify-center mx-auto gap-2"
          >
        ...
        <div className="bg-white/5 backdrop-blur-md rounded-[2rem] p-8 border border-white/10 text-left">
          {benefitsList.map((section, sidx) => (
            <div key={sidx} className="mb-6 last:mb-0">
               <h4 className="text-velonet-orange font-black text-sm uppercase mb-4 flex items-center gap-2">
                 <div className="w-1.5 h-1.5 bg-velonet-orange rounded-full"></div>
                 {section.title}
               </h4>
               <ul className="space-y-3">
                 {section.items.map((item, iidx) => (
                   <li key={iidx} className="flex items-start gap-3 text-white/70 text-xs font-bold leading-relaxed">
                     <Check size={14} className="text-velonet-orange flex-shrink-0 mt-0.5" />
                     {item}
                   </li>
                 ))}
               </ul>
...
const StratumHeader = ({ subtitle, title, arrowText }) => (
  <div className="lg:w-1/3 text-left">
    <div className="mb-8">
      <div className="w-16 h-16 bg-velonet-orange/20 rounded-2xl flex items-center justify-center mb-10 shadow-[0_0_30px_rgba(255,170,0,0.2)]">
        <svg viewBox="0 0 24 24" className="w-10 h-10 text-velonet-orange fill-current">
      ...
      <p className="text-velonet-orange font-black uppercase text-[9px] tracking-[0.2em] mb-4 border-l-2 border-velonet-orange pl-3 max-w-[280px]">
        {subtitle}
      </p>
      <h2 className="text-2xl md:text-4xl font-black text-white mb-10 leading-[1.2] tracking-tight italic">
        {title}
      </h2>
      <button className="flex items-center gap-4 text-white group bg-white/5 pr-8 pl-2 py-2 rounded-full border border-white/10 hover:border-velonet-orange transition-all">
        <div className="w-12 h-12 rounded-full bg-velonet-orange flex items-center justify-center shadow-lg shadow-velonet-orange/30">
          <ArrowRight className="text-velonet-purple" size={24} strokeWidth={3} />
        </div>
        <span className="font-black uppercase text-[10px] tracking-widest group-hover:text-velonet-orange transition-colors">
          {arrowText}
        </span>
      </button>
    </div>
...
export default function PlansSection() {
  ...
  return (
    <section id="planes" className="py-24 md:py-32 px-4 md:px-8 bg-velonet-purple relative overflow-hidden">
      {/* Background Decor */}
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-full h-full bg-[radial-gradient(circle_at_center,_var(--tw-gradient-stops))] from-velonet-purple-light/10 via-transparent to-transparent opacity-50"></div>
